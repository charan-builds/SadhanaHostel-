-- Fix resident purge to release room allocations using the actual schema.
...
begin
  ...
  set is_active = false, deleted_at = v_deleted_at, deleted_by = p_actor_user_id,
      updated_at = v_deleted_at, updated_by = p_actor_user_id
  where organization_id = p_organization_id
    and resident_id = p_resident_id and deleted_at is null;

  update public.advance_payment_allocations
  set deleted_at = v_deleted_at, deleted_by = p_actor_user_id,
      updated_at = v_deleted_at, updated_by = p_actor_user_id
  where organization_id = p_organization_id
    and resident_id = p_resident_id and deleted_at is null;
  ...
  where payment_id in (
    ...
    where organization_id = p_organization_id and resident_id = p_resident_id
  );
  ...
  set status = 'completed'::public.room_allocation_status_enum,
      allocated_to = coalesce(allocated_to, v_deleted_at::date),
      reason = coalesce(reason, 'Resident deleted from operations.'),
  ...
  set status = 'archived'::public.resident_status_enum,
      is_active = false, user_id = null, parent_user_id = null,
      deleted_at = v_deleted_at, deleted_by = p_actor_user_id,
      updated_at = v_deleted_at, updated_by = p_actor_user_id
  where id = p_resident_id and organization_id = p_organization_id;
