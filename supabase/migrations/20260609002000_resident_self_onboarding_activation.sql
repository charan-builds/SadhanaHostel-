-- Complete resident self-onboarding as one guarded lifecycle transition.
-- The application authorizes the resident session first, then invokes this
-- service-role-only RPC. The database revalidates identity, portal access,
-- lifecycle state, and every required profile field before activation.
...
begin
  if new.onboarding_status = 'verified'
     and coalesce(
       current_setting('app.resident_self_onboarding_completion', true),
       ''
     ) <> 'true'
...
comment on function public.complete_resident_self_onboarding_atomic(
  ...
  'Atomically activates a fully completed, auth-linked resident after verifying active resident portal access. Callable only by trusted server service-role code.';
