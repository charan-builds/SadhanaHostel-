-- Atomic, audited financial corrections.
--
-- Paid admission fee corrections synchronize the resident, fee record,
-- payment, and invoice. Advance corrections remain append-only liability
-- movements. Every correction writes an immutable resident-scoped audit row.
...
begin
  ...
  if p_change_type = 'monthly_fee' then
    ...
    order by
      (
        coalesce((mfr.metadata ->> 'generated_for_initial_collection')::boolean, false)
      ) desc,
      mfr.created_at asc
    limit 1
    for update;
    ...
    if v_fee_record.id is not null then
      ...
      where p.organization_id = p_organization_id
        ...
        and p.is_advance = false
        and p.deleted_at is null
      order by
        (
          coalesce((p.metadata ->> 'first_month_fee')::boolean, false)
        ) desc,
...
comment on function public.apply_resident_financial_correction_atomic(
  uuid,
  uuid,
  text,
  numeric,
  ...
  'Service-role-only atomic correction of a resident monthly fee or advance balance with immutable audit history.';
