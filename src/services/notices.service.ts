...
export class NoticesService {
  ...
  async updateNotice(input: unknown) {
    ...
    const updates: TablesUpdate<"notices"> = {
      updated_by: context.authUser.id,
    }
    ...
    if (values.status !== undefined) {
      ...
      if (values.status === "published") {
        ...
        updates.published_by = context.authUser.id
      }
    }

    if (values.noticeType !== undefined) {
      updates.notice_type = values.noticeType
    }

    if (values.requiresAcknowledgement !== undefined) {
      updates.requires_acknowledgement = values.requiresAcknowledgement
    }

    if (values.audienceType !== undefined) {
      updates.audience_type = values.audienceType
    }

    if (values.audienceFilter !== undefined) {
      updates.audience_filter = values.audienceFilter as Json
    }

    if (values.isPinned !== undefined) {
      updates.is_pinned = values.isPinned
    }
    ...
    if (values.expiresAt !== undefined) {
      updates.expires_at = values.expiresAt
    }

    const notice = await this.noticesRepository.update(
      values.noticeId,
      values.organizationId,
      updates
    )
